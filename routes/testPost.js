 ...
 * 게시글 수정 권한 확인
 */
router.get("/:postIdx/iswriter", (req, res) => {
 ...
 * 댓글 수정 권한 확인
 */
router.get("/:postIdx/comment/:commentIdx/iswriter", (req, res) => {
  return res.status(StatusCodes.OK).send(StatusCodes.OK);
});
 ...
 * 댓글 수정
 */
router.patch("/:postIdx/comment/:commentIdx", async (req, res) => {
  try {
 ...
 * 댓글 삭제
 */
router.delete("/:postIdx/comment/:commentIdx", async (req, res) => {
  try {
    const commentIdx = req.params.commentIdx;
